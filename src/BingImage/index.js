...
let getFilePath = (enddate, fileType) => {
    ...
    let filePath = './src/BingImage/' + enddate.substring(0, 4) + (fileType === '.jpg' ? '/' + enddate.substring(0, 6) : '');
    try {
...
`;
  ...
  link: /BingImage/${prevMonth.getFullYear()}/${prevMonth.getFullYear()}${(prevMonth.getMonth() < 9 ? '0' : '') + (prevMonth.getMonth() + 1)}.html
`;
...
`;
  ...
  link: /BingImage/${thisDate.getFullYear()}/${thisDate.getFullYear()}${(thisDate.getMonth() < 9 ? '0' : '') + (thisDate.getMonth() + 1)}.html
`;
    } else if (nextMonth.getFullYear() >= 2020) {
        frontmatter += `next:
  text: ${nextMonth.getFullYear()}/${nextMonth.getMonth() < 9 ? '0' : ''}${nextMonth.getMonth() + 1}
  link: /BingImage/${nextMonth.getFullYear()}/${nextMonth.getFullYear()}${(nextMonth.getMonth() < 9 ? '0' : '') + (nextMonth.getMonth() + 1)}.html
`;
    }
    frontmatter += `
---
                ...
                .then(json => {
                    if (json.MediaContents) {
                        ...
                        updateMDFile(lastMediaContents.sort((a, b) => {
                            return getEnddate(a) < getEnddate(b) ? 1 : -1;
                        }).slice(0, 15), './src/BingImage/README.md');
